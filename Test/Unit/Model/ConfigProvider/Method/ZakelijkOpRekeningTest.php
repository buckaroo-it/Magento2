...
class ZakelijkOpRekeningTest extends BaseTest
{
    protected $instanceClass = ZakelijkOpRekening::class;
    ...
    public function testIsInactive()
    {
        ...
        $scopeConfigMock->method('getValue')
            ->with(
                $this->getPaymentMethodConfigPath(ZakelijkOpRekening::CODE, AbstractConfigProvider::ACTIVE)
            )
            ->willReturn(0);
    ...
    public function testGetConfig(): void
    {
        ...
        $this->assertArrayHasKey(ZakelijkOpRekening::CODE, $result['payment']['buckaroo']);
        $this->assertArrayHasKey(
            'allowedCurrencies',
            $result['payment']['buckaroo'][ZakelijkOpRekening::CODE]
        );
        $this->assertArrayHasKey(
            'tooltipUrl',
            $result['payment']['buckaroo'][ZakelijkOpRekening::CODE]
    ...
    public function testGetPaymentFee($fee, $expected)
    {
        ...
        $scopeConfigMock->method('getValue')
            ->with(
                ZakelijkOpRekening::XPATH_PAYMENT_FEE,
                ScopeInterface::SCOPE_STORE
            )->willReturn($fee);
