...
use Buckaroo\Magento2\Model\Method\Belfius;

class BelfiusTest extends \Buckaroo\Magento2\Test\BaseTest
{
    ...
    public function testGetOrderTransactionBuilder()
    {
        ...
        $orderMock->expects($this->once())->method('setServices')->willReturnCallback(
            function ($services) use ($fixture, $orderMock) {
                $this->assertEquals('belfius', $services['Name']);
                $this->assertEquals('Pay', $services['Action']);
                $this->assertEquals(0, $services['Version']);
                return $orderMock;
            }
        );
    ...
    public function testGetRefundTransactionBuilder()
    {
        ...
        $paymentMock->expects($this->once())
            ...
            ->with(Belfius::BUCKAROO_ORIGINAL_TRANSACTION_KEY_KEY)
    ...
    /**
     * @covers ::getPaymentMethodName()
     */
    public function testGetPaymentMethodName()
    {
        $infoInterface = $this->getFakeMock(InfoInterface::class)->getMockForAbstractClass();
        $instance = $this->getInstance();
        $this->assertEquals('belfius', $instance->getPaymentMethodName($infoInterface));
    }
}
