...
class RefundGiftcard extends AbstractTransactionBuilder
{
     ...
     * @return self
     */
    public function setGiftcardTransaction(GroupTransaction $giftcardTransaction)
    {
    ...
    public function getBody()
    {
        ...
        if (!$this->giftcardTransaction instanceof GroupTransaction) {
            throw new \Exception("`giftcardTransaction` must be instance of Buckaroo\Magento2\Model\GroupTransaction");
        }

        $ip = $this->getUserIp($this->store);

        $body = [
            'Currency' => $this->giftcardTransaction->getCurrency(),
            'AmountDebit' => 0,
            'AmountCredit' => $this->giftcardTransaction->getRemainingAmount(),
            'Invoice' => $this->giftcardTransaction->getOrderIncrementId(),
            'Order' => $this->giftcardTransaction->getOrderIncrementId(),
            ...
            'OriginalTransactionKey' => $this->giftcardTransaction->getTransactionId(),
            'StartRecurrent' => $this->startRecurrent,
            'PushURL' => $this->urlBuilder->getDirectUrl('rest/V1/buckaroo/push'),
            'Services' => (object)[
                'Service' => [
                    'Name'             =>  $this->giftcardTransaction->getServicecode(),
                    'Action'           => 'Refund',
                ]
            ],
