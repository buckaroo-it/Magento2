 ...
 *
 * Klarna refunds must reference the Pay transaction, never the Reserve DataRequest key. This
 * handler makes the synchronous capture response the authoritative writer of
 * `buckaroo_capture_transaction_key`, which the refund request builder prefers (BTI-1267).
 *
 * Composed alongside (not in place of) the shared capture handler chain, so it only adds the
 * capture-specific keys and leaves the generic transaction id handling untouched.
 */
